...
async function optimiseImage(absSrc: string, slug: string, baseDir: string) {
  const relWithinSlug = path.relative(baseDir, absSrc);
...
(async () => {
  ...
  const targets: { slug: string; dir: string }[] = [];

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;

    if (entry.name === "interactive") {
      const interactiveDir = path.join(POSTS_DIR, "interactive");
      const subEntries = await fs.readdir(interactiveDir, {
        withFileTypes: true,
      });
      for (const sub of subEntries) {
        if (sub.isDirectory()) {
          targets.push({
            slug: sub.name,
            dir: path.join(interactiveDir, sub.name),
          });
        }
      }
    } else {
      targets.push({ slug: entry.name, dir: path.join(POSTS_DIR, entry.name) });
    }
  }
  ...
  for (const { slug, dir } of targets) {
    for await (const img of walk(dir)) {
      jobs.push(
        limit(async () => {
          const delta = await optimiseImage(img, slug, dir);
