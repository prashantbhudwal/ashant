...
/**
 * Parses markdown content to extract ## Context section and ```md code block
 */
function parsePromptContent(content: string): {
  ...
  // Extract content inside ```md ... ``` code block
  const codeBlockMatch = /```md\s*\n([\s\S]*?)```/.exec(content);
  const prompt = codeBlockMatch?.[1]?.trim() || "";

  return { context: context || undefined, prompt };
}
...
const prompts = defineCollection({
  ...
  transform: async (document) => {
    const { context, prompt } = parsePromptContent(document.content);
    return {
      ...document,
      context,
      prompt,
